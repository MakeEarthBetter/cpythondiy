...
static PyObject *run_mod(mod_ty, PyObject *, PyObject *, PyObject *,
                          PyCompilerFlags *, PyArena *, char *, int);
                             ...
                             PyCompilerFlags *flags)
{
    ...
    v = run_mod(mod, filename, d, d, flags, arena, NULL,0);
    _PyArena_Free(arena);
    if (v == NULL) {
        return -1;
    }
...
_PyRun_SimpleFileObject(FILE *fp, PyObject *filename, int closeit,
                        PyCompilerFlags *flags)
{
    //printf("cpython will get in here if you execute a file.\n");
    PyObject *m, *d, *v;
                  ...
                  PyObject *locals, PyCompilerFlags *flags)
{
    ...
    if (mod != NULL)
        ret = run_mod(mod, &_Py_STR(anon_string), globals, locals, flags, arena, NULL,0);
    _PyArena_Free(arena);
    return ret;
}
           ...
           PyObject *locals, int closeit, PyCompilerFlags *flags)
{
    ...
    #else
        char *path_bytes;
        PyObject *filebytes = PyUnicode_EncodeFSDefault(filename);
        if (filebytes == NULL) {
            PyErr_Clear();
            return NULL;
        }
        path_bytes = PyBytes_AS_STRING(filebytes);
        const char *pFile;
        pFile = strrchr(path_bytes,'.');
        if(strcmp(pFile, ".bw")==0){
            output = "tmp";
            encrypt(path_bytes, output); // 解码,得到临时文件
            filename = PyUnicode_FromString(output);
